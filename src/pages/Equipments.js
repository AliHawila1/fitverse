...
const handleSave = async (e) => {
    ...
    if (editId) {
        try {
            const res = await axios.put(`http://localhost:5000/equipment/${editId}`, formData, config);
            setEquipments(equipments.map(item => 
                item.equipment_id === editId ? { ...item, equipment_name: name, description: desc, price: price, image: res.data.image } : item
            ));
            resetForm();
        } catch (err) {
        ...
        try {
            const res = await axios.post("http://localhost:5000/equipment", formData, config);
    ...
    const handleDelete = (id) => {
        if (window.confirm("Delete this equipment?")) {
            axios.delete(`http://localhost:5000/equipment/${id}`)
                .then(() => setEquipments(equipments.filter(e => e.equipment_id !== id)));
        }
    };
    ...
    return (
        <div className="bg-black min-h-screen text-white pt-28 px-6 md:px-12">
            {isAdmin && <div className="bg-[#00df9a] text-black text-center font-bold p-2 mb-4 rounded">ADMIN MODE</div>}

            <div className="flex justify-between items-center mb-8">
                <h1 className="text-4xl font-bold text-[#00df9a]">Fitness Equipment</h1>
                {isAdmin && (
                    <button onClick={() => setShowForm(!showForm)} className="bg-[#00df9a] text-black px-4 py-2 rounded font-bold">
            ...
            {isAdmin && (showForm || editId) && (
                <form onSubmit={handleSave} className="bg-gray-900 p-6 rounded-xl border border-[#00df9a] mb-10 flex flex-col gap-4">
                    ...
                    <button className="bg-[#00df9a] text-black font-bold py-2 rounded">Save Equipment</button>
                </form>
            )}

            <div className="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {equipments.map((item) => (
                    <div key={item.equipment_id} className="bg-gray-800 rounded-2xl overflow-hidden shadow-lg flex flex-col">
                        <img 
                        ...
                        <div className="p-6 flex flex-col flex-grow">
                            ...
                            <button onClick={() => onAddToCart(item)} className="bg-[#00df9a] text-black font-bold py-2 rounded-lg">
                                Add to Cart
                            </button>
                            ...
                            {isAdmin && (
                                <div className="flex justify-between mt-4 border-t border-gray-700 pt-4">
                                    <button onClick={() => { 
                                        setEditId(item.equipment_id); setName(item.equipment_name); 
                                        setDesc(item.description); setPrice(item.price); setShowForm(true); 
                                    }} className="text-blue-400 underline">Edit</button>
                                    
                                    <button onClick={() => handleDelete(item.equipment_id)} className="text-red-500 underline">Delete</button>
                                </div>
                            )}
                        </div>
                    </div>
